...
public:
    ...
    WebSocketList list_ws;
    //    WebSocketMap list_client_ws;    
    std::shared_ptr<Poco::Net::WebSocket> ws_iter;
};
...
bool ZApiWS::initialize() {
    ...
    worker->receiveMessageEvent += Poco::delegate(this, &ZApiWS::onDeviceToWSDataUpdated);
...
void ZApiWS::handle_client_login(std::shared_ptr<Poco::Net::WebSocket> websocket, const ServerPacket& clientData) {
    ...
    //    app.logger().information("Total ws first: " + std::to_string(d_ptr->list_client_ws.size()));
    //    if (iLookup == d_ptr->list_client_ws.end()) {        
    //        d_ptr->list_ws.push_back(websocket);
    //        d_ptr->list_client_ws[info.getApikey()] = d_ptr->list_ws;
    //    } else {
    //        iLookup->second.push_back(websocket);
    //        d_ptr->list_client_ws[info.getApikey()] = iLookup->second;
    //    }
    d_ptr->list_ws.push_back(websocket);
    //    d_ptr->list_client_ws[info.getApikey()] = d_ptr->list_ws;
    app.logger().information("Total ws: " + std::to_string(d_ptr->list_ws.size()));
}
...
void ZApiWS::handle_client_logout(std::shared_ptr<Poco::Net::WebSocket> websocket) {
    for (ZApiWS::Impl::WebSocketList::iterator iter = d_ptr->list_ws.begin(); iter != d_ptr->list_ws.end(); ++iter) {
        if (*iter == websocket) {
            d_ptr->list_ws.erase(iter);
            break;
        }
    }
}
...
void ZApiWS::send_client_list(const std::string& apikey, const std::string& msg_out) {
    ...
    try {
        ...
        //        if (iLookup == d_ptr->list_client_ws.end()) {
        //            app.logger().information("Not found client " + apikey);
        //            return;
        //        }

        for (std::shared_ptr<Poco::Net::WebSocket> ws_iter : d_ptr->list_ws) {
            d_ptr->ws_iter = ws_iter;
            ws_iter->sendFrame(msg_out.c_str(), msg_out.length(), WebSocket::FRAME_TEXT);
        }
    } catch (Poco::Exception &ex) {
        Application& app = Application::instance();
        app.logger().error("ZApiWS::send_client_list. Exception while processing message: %s", ex.displayText());
        this->handle_client_logout(d_ptr->ws_iter);
    }
}
