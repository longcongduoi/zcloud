...
void ZApiWS::onDeviceToWSDataUpdated(const void* pSender, const ZCloudReceiveMessageEvent& event) {
    Application& app = Application::instance();
    try {               
        std::string msg  = event.msg();
        Poco::JSON::Object::Ptr result = StringUtil::convertString2Json(msg);       
        std::string userApiKey = result->optValue("userApiKey", std::string());
        std::string content = result->optValue("content", std::string());
        send_client_list(userApiKey, content);
    } catch (Poco::Exception &ex) {
        Application& app = Application::instance();
        app.logger().error("ZApiWS::onDeviceToWSDataUpdated. Exception while processing message: %s", ex.displayText());
    }
    app.logger().information("broadcast finish");
}
...
void ZApiWS::onDeviceDataUpdated(const void *pSender, const ZDeviceDataUpdatedEvent &event) {
    Application& app = Application::instance();
    try {        
        app.logger().information("value: %s", event.varialbleValue());
        ...
        Application& app = Application::instance();
        app.logger().error("ZApiWS::onDeviceDataUpdated. Exception while processing message: %s", ex.displayText());
    }
    app.logger().information("broadcast finish");
}
